...
async function validateCampaign(campaign) {
    ...
    for (const { variantId } of products) {
        ...
        const logic = campaign.discountLogic;
        ...
        if (expectedPct !== logic.value) {
            errors.push(`Variant ${variantId}: expected discount ${expectedPct}% off reference ${reference.toString()}, but campaign logic is ${logic.value}%.`);
        }
    }
    return errors;
}
